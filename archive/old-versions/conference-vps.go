...
// HTML client embedded in Go - updated for VPS
...
<body>
    ...
    <script>
        // Configuration - use window location for flexibility
...
func main() {
	...
	if useSSL {
		log.Printf("Conference server starting with HTTPS on port %s", port)
		log.Printf("Access at https://194.87.103.57:%s", port)
		if err := http.ListenAndServeTLS("0.0.0.0:"+port, certFile, keyFile, nil); err != nil {
			log.Fatal("ListenAndServeTLS: ", err)
		}
	} else {
		log.Printf("Conference server starting with HTTP on port %s", port)
		log.Printf("Access at http://194.87.103.57:%s", port)
